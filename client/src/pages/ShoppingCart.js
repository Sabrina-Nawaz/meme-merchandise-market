      ...
      <Container class="m-5">
        <h2>
          {userData.savedProducts.length
            ? `Viewing ${userData.savedProducts.length} saved ${
                userData.savedProducts.length === 1 ? "product" : "products"
              }:`
          ...
          {userData.savedProducts.map((product) => {
            return (
              <Card key={product.bookId} border="dark">
                {product.image ? (
                  <Card.Img
                    src={`img/shirts/${product.image}`}
                    alt={`The cover for ${product.productName}`}
                    variant="top"
                  />
                ) : null}
                <Card.Body>
